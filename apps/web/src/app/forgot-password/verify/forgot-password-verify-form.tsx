...
export function ForgotPasswordVerifyForm() {
  ...
  const { verifyResetOTP, resendVerification } = useCustomerAuth();

  const challengeIdParam = searchParams.get("challengeId");
  const emailParam = searchParams.get("email");

  const challengeId = challengeIdParam ? parseInt(challengeIdParam, 10) : undefined;
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      await verifyResetOTP(challengeId, otp);
  ...
  const handleResend = async () => {
    ...
    try {
      await resendVerification({ challengeId, email: emailParam || undefined });
      setSuccessMsg("If an account exists for this email, we've sent a verification code.");
      setCooldown(60);
    } catch (err: any) {
  ...
  return (
    <div className="mx-auto my-12 max-w-md rounded-2xl border border-deep-brown bg-white p-6 shadow-md md:p-8">
      <div className="text-center">
        <h2 className="font-baloo text-3xl font-extrabold text-deep-brown">Enter Reset Code</h2>
        <p className="mt-2 text-text-primary">
          If an account exists for this email, we&apos;ve sent a verification code. Enter the code below if you received it.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-8 space-y-6">
        ...
        >
          {formLoading ? "Verifying..." : "Authorize Password Reset"}
        </button>
      </form>
      ...
      <div className="mt-6 text-center text-sm text-text-primary">
        ...
        <button
          ...
          disabled={!canResend || formLoading}
