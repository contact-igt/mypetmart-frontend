...
describe("POST /order/payment/success (PayU browser return)", () => {
  it("redirects to the non-authoritative result page with status=success", async () => {
    const res = await POST(postRequest("https://storefront.example.com/order/payment/success", { txnid: "PAY-000123", udf1: "42", status: "success" }));
    expect(res.status).toBe(303);
    const location = new URL(res.headers.get("location")!);
    expect(location.pathname).toBe("/order/payment/result");
    expect(location.searchParams.get("status")).toBe("success");
    expect(location.searchParams.get("txnid")).toBe("PAY-000123");
  ...
  it("never sets status to anything but 'success' regardless of PayU's own posted status field", async () => {
    ...
    const res = await POST(postRequest("https://storefront.example.com/order/payment/success", { txnid: "PAY-000123", udf1: "42", status: "failure" }));
    const location = new URL(res.headers.get("location")!);
    expect(location.searchParams.get("status")).toBe("success");
  });
  ...
  it("drops a non-numeric orderId(udf1) instead of reflecting it", async () => {
    const res = await POST(postRequest("https://storefront.example.com/order/payment/success", { txnid: "PAY-000123", udf1: "not-a-number" }));
    const location = new URL(res.headers.get("location")!);
  ...
  it("also handles a defensive GET fallback, reading fields from the query string", async () => {
    const res = await GET(new NextRequest("https://storefront.example.com/order/payment/success?txnid=PAY-000456&udf1=7"));
    expect(res.status).toBe(303);
    const location = new URL(res.headers.get("location")!);
    expect(location.searchParams.get("txnid")).toBe("PAY-000456");
