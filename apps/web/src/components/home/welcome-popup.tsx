...
function TemplateTwo({ popup, onDismiss, action, showConsent }: ContentProps) {
  ...
  return <div className="grid max-h-[min(42rem,calc(100vh-2rem))] overflow-y-auto sm:min-h-[30rem] sm:grid-cols-2"><div className="min-h-56 bg-primary-orange sm:h-full"><PopupImage popup={popup} rounded={false} /></div><div className="relative flex flex-col justify-center bg-deep-brown p-6 text-white sm:p-8"><CloseButton onDismiss={onDismiss} /><h2 id={`welcome-popup-title-${popup.id}`} className="pr-8 text-3xl font-bold leading-[0.95] tracking-[-0.045em] text-white sm:text-4xl">{popup.heading}</h2>{popup.offerLabel && <p className="mt-5 text-3xl font-bold leading-[0.95] tracking-[-0.045em] text-primary-orange sm:text-4xl">{popup.offerLabel}</p>}{popup.ctaMode === "navigation" && description}<div className="[&>form>input]:border-0 [&>form>button]:rounded-xl [&>form>button]:bg-white [&>form>button]:text-deep-brown [&>a]:rounded-xl [&>a]:bg-white [&>a]:text-deep-brown">{action}</div>{popup.couponCode && <CouponOffer code={popup.couponCode} dark />}{popup.ctaMode !== "navigation" && description}{showConsent && popup.consentText && <p className="mt-3 text-[11px] leading-relaxed text-white/65">{popup.consentText}</p>}{popup.dismissLabel && <button type="button" onClick={onDismiss} className="mt-5 text-sm font-semibold text-white underline underline-offset-4 hover:no-underline">{popup.dismissLabel}</button>}</div></div>;
}
